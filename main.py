...
from help_functionality import help_functionality  # Import the function
...
class Face_Recognition:
    def __init__(self, root):
        ...
        img_help = Image.open(r"assets\developer.jpg")
        img_help = img_help.resize((180, 180), Image.LANCZOS)
        ...
        recognizer_x_position = (screen_width / 5)
        results_x_position = (2 * screen_width / 5)
        help_x_position = (3 * screen_width / 5)
        exit_x_position = (4 * screen_width / 5)

        vertical_image_offset = screen_height / 4
        vertical_button_offset = (screen_height / 4) + 200

        # Place Recognizer elements
        self.label_recognizer = Label(self.root, image=self.photoimg_recognizer, cursor="hand2", bd=0)
        self.label_recognizer.place(x=recognizer_x_position, y=vertical_image_offset, anchor="center")
        self.label_recognizer.bind("<Button-1>", lambda event: on_image_recognizer_click())

        self.button_recognizer = Button(self.root, text="Face Recognizer", cursor="hand2", bg="#1f2833", fg="white", bd=0, command=on_image_recognizer_click, width=15, height=1, font=button_font)
        self.button_recognizer.place(x=recognizer_x_position, y=vertical_button_offset, anchor="center")

        # Place Results elements
        self.label_results = Label(self.root, image=self.photoimg_results, cursor="hand2", bd=0)
        self.label_results.place(x=results_x_position, y=vertical_image_offset, anchor="center")
        ...
        self.button_results.place(x=results_x_position, y=vertical_button_offset, anchor="center")

        # Place Help elements
        self.label_help = Label(self.root, image=self.photoimg_help, cursor="hand2", bd=0)
        self.label_help.place(x=help_x_position, y=vertical_image_offset, anchor="center")
        self.label_help.bind("<Button-1>", lambda event: on_help_click())

        self.button_help = Button(self.root, text="Help", cursor="hand2", bg="#1f2833", fg="white", bd=0, command=on_help_click, width=15, height=1, font=button_font)
        self.button_help.place(x=help_x_position, y=vertical_button_offset, anchor="center")

        # Place Exit elements
        self.label_exit = Label(self.root, image=self.photoimg_exit, cursor="hand2", bd=0)
        self.label_exit.place(x=exit_x_position, y=vertical_image_offset, anchor="center")
        ...
        self.button_exit.place(x=exit_x_position, y=vertical_button_offset, anchor="center")
