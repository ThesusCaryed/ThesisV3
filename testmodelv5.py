...
def main():
    ...
    while True:
        ...
        if boxes:
            ...
            recognized_names = []  # List to store names of recognized faces
            ...
            for box in selected_boxes:
                ...
                if confidence < MIN_CONFIDENCE_FOR_RECOGNITION:
                    ...
                    recognized_names.append(name if name != "Unknown" else f"Unknown ({accuracy:.2f}%)")
                else:
                    recognized_names.append(f"Unknown ({accuracy:.2f}%)")
            ...
            if recognized_names:
                label = ', '.join(recognized_names)
                print(f"Recognized Names: {label}")

                current_time = get_philippine_time()
                timestamp = current_time.strftime('%Y-%m-%d %H:%M:%S')
                display_text = f"{timestamp} - {label}"
